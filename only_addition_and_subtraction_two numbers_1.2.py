...
import random
"""
!!! ВАЖЛИВО. НЕ ПРАЦЮЄ ЗМІНА ЗНАКУ + ЧИ -, ЯКЩО ПЕРШИЙ ПРИКЛАД ДОДАВАННЯ - ВСІ НАСТУПНІ НА ДОДАВАННЯ, І НАВПАКИ, З ВІДНІМАННЯМ ЯКЩО ПЕРШИЙ БУВ НА ВІДНІМАННЯ
...
def upd_Text():
    ...
    """
    global number, number2, result, formula, count
    number = rand_number1()
